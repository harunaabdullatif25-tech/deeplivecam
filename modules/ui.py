...
# Scale factor applied to camera frames before swap inference on CPU.
# Halving the linear dimension cuts affine-warp and paste-back cost by ~4×
# while the inswapper model itself always processes 128×128 internally.
_CPU_LIVE_SCALE = 0.5


class _DetectionState:
    """Thread-safe container for the latest face-detection results."""

    def __init__(self):
        self._lock = threading.Lock()
        self._target_face = None
        self._many_faces = None

    def update(self, target_face, many_faces) -> None:
        with self._lock:
            self._target_face = target_face
            self._many_faces = many_faces

    def get(self):
        """Return (target_face, many_faces) atomically."""
        with self._lock:
            return self._target_face, self._many_faces
...
class _DetectionWorker(QThread):
    """Continuously detects faces in the latest raw frame.

    Runs fully asynchronously from _ProcessingWorker so detection never
    blocks the swap step.  On CPU this is the biggest single win: a 300 ms
    detection no longer stalls every swap cycle.
    """

    def __init__(
        self,
        det_queue: queue.Queue,
        det_state: _DetectionState,
        ...
        super().__init__()
        self._dq = det_queue
        self._state = det_state
        self._stop = stop_event
        self._cpu_only = cpu_only
        self._scale = live_scale

    def run(self) -> None:
        while not self._stop.is_set():
            try:
                frame = self._dq.get(timeout=0.1)
            except queue.Empty:
                continue

            # Down-scale before detection to match the resolution the
            # processing worker will swap at.
            if self._cpu_only and self._scale < 1.0:
                h, w = frame.shape[:2]
            ...
            try:
                if modules.globals.many_faces:
                    ...
                    self._state.update(None, faces)
                else:
                    face = detect_one_face_fast(frame)
                    self._state.update(face, None)
            except Exception:
                pass  # never kill the detection thread on transient errors


class _ProcessingWorker(QThread):
    """Pulls raw frames, runs swap/enhance using async-detected faces, pushes results.

    On CPU-only hardware the worker:
    - Downscales frames by _CPU_LIVE_SCALE before swap (4× fewer warp pixels)
    - Reads face positions from _DetectionState (filled by _DetectionWorker)
      so detection never stalls the swap step
    On GPU hardware the old inline detection path is preserved.
    """
    ...
    def _push_det_frame(self, frame) -> None:
        """Feed a frame to the detection worker, dropping stale ones."""
        try:
            self._dq.put_nowait(frame)
        except queue.Full:
            try:
                self._dq.get_nowait()
            except queue.Empty:
    ...
    def run(self) -> None:
        ...
        # Legacy inline-detection state (used on GPU path only)
        det_count = 0
        cached_target_face = None
        cached_many_faces = None
        det_interval = max(1, round(self._fps * 0.08))

        live_scale = _CPU_LIVE_SCALE if self._cpu_only else 1.0
        ...
        while not self._stop.is_set():
            ...
            if not modules.globals.map_faces:
                ...
                if self._cpu_only:
                    # --- CPU path: async detection + frame downscaling ---
                    # Feed the current (full-res) frame to the detection worker.
                    self._push_det_frame(temp_frame)

                    # Downscale for cheaper warp/paste-back.
                    orig_h, orig_w = temp_frame.shape[:2]
                    if live_scale < 1.0:
                        new_w = max(1, int(orig_w * live_scale))
                        new_h = max(1, int(orig_h * live_scale))
                        swap_frame = cv2.resize(
                            temp_frame, (new_w, new_h), interpolation=cv2.INTER_LINEAR
                        )
                    else:
                        swap_frame = temp_frame

                    # Read the latest detected faces (may be None on first frame).
                    cached_target_face, cached_many_faces = self._det_state.get()

                    cached_faces = None
                    if modules.globals.many_faces and cached_many_faces:
                        cached_faces = cached_many_faces
                    elif not modules.globals.many_faces and cached_target_face is not None:
                        cached_faces = [cached_target_face]

                    if modules.globals.mouth_mask and cached_faces:
                        ensure_landmarks(swap_frame, cached_faces)

                    for fp in frame_processors:
                        if fp.NAME == "DLC.FACE-ENHANCER":
                            if modules.globals.fp_ui["face_enhancer"]:
                                swap_frame = fp.process_frame(
                                    None, swap_frame, detected_faces=cached_faces
                                )
                            ...
                            if modules.globals.many_faces and cached_many_faces:
                                ...
                                swap_frame = fp.swap_face(
                                    source_image, cached_target_face, swap_frame
                                )
                                if (
                            ...
                            swap_frame = fp.apply_post_processing(swap_frame, swapped_bboxes)
                        else:
                            swap_frame = fp.process_frame(source_image, swap_frame)

                    # Upscale back to camera resolution for display.
                    if live_scale < 1.0:
                        temp_frame = cv2.resize(
                            swap_frame, (orig_w, orig_h), interpolation=cv2.INTER_LINEAR
                        )
                    else:
                        temp_frame = swap_frame

                else:
                    # --- GPU path: inline detection (original logic) ---
                    det_count += 1
...
class WebcamPreviewWindow(QWidget):
    def __init__(self, camera_index: int):
        ...
        # Detect whether we are running on CPU only
        _gpu_providers = {
        ...
        cpu_only = not any(
            p in _gpu_providers for p in modules.globals.execution_providers
        )
        if cpu_only:
            print("[webcam] CPU-only mode: async detection + frame downscaling active")

        self._capture_queue: queue.Queue = queue.Queue(maxsize=2)
        self._processed_queue: queue.Queue = queue.Queue(maxsize=2)
        # Single-slot queue: detection worker only ever needs the latest frame.
        self._det_queue: queue.Queue = queue.Queue(maxsize=1)
        self._det_state = _DetectionState()
        ...
        self._detection_worker = _DetectionWorker(
            ...
            self._stop_event,
            cpu_only=cpu_only,
            live_scale=_CPU_LIVE_SCALE if cpu_only else 1.0,
        )
    ...
    def closeEvent(self, event) -> None:
        ...
        try:
            ...
            pass
        for worker in (self._capture_worker, self._detection_worker, self._processing_worker):
            try:
                worker.wait(2000)
            except Exception:
                pass
