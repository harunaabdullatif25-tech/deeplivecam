...
# Dynamically chosen in _get_det_size() — do not reference DET_SIZE directly.
_DET_SIZE_GPU = (640, 640)
_DET_SIZE_CPU = (320, 320)  # ~4× fewer pixels → ~2–3× faster on CPU


def _is_cpu_only() -> bool:
    """Return True when no hardware-accelerated execution provider is active."""
    _GPU_PROVIDERS = {
    ...
    return not any(p in _GPU_PROVIDERS for p in modules.globals.execution_providers)


def _get_det_size() -> tuple:
    """Return the optimal detection size for the current execution provider."""
    return _DET_SIZE_CPU if _is_cpu_only() else _DET_SIZE_GPU


DET_SIZE = _DET_SIZE_GPU  # kept for backward-compat imports
    ...
    if FACE_ANALYSER is None:
        with FACE_ANALYSER_LOCK:
            ...
            if FACE_ANALYSER is None:
                ...
                _det_size = _get_det_size()
                FACE_ANALYSER.prepare(ctx_id=0, det_size=_det_size)
                _optimize_det_model(FACE_ANALYSER, providers, _det_size)
    return FACE_ANALYSER


def _optimize_det_model(fa: Any, providers, det_size: tuple = _DET_SIZE_GPU) -> None:
