    ...
    with THREAD_LOCK:
        if FACE_SWAPPER is None:
            ...
            try:
                ...
                # CPU-only: replace the insightface-default session with a
                # tuned one that uses all available cores and enables memory
                # pattern reuse, giving a meaningful boost at no quality cost.
                _is_cpu_only_provider = not any(
                    p in ('CUDAExecutionProvider', 'ROCMExecutionProvider',
                          'DmlExecutionProvider', 'CoreMLExecutionProvider',
                          'OpenVINOExecutionProvider')
                    for p in modules.globals.execution_providers
                )
                if _is_cpu_only_provider and hasattr(FACE_SWAPPER, 'session'):
                    try:
                        import onnxruntime as _ort
                        _cpu_opts = _ort.SessionOptions()
                        _cpu_opts.graph_optimization_level = (
                            _ort.GraphOptimizationLevel.ORT_ENABLE_ALL
                        )
                        _cpu_opts.intra_op_num_threads = max(1, (os.cpu_count() or 1))
                        _cpu_opts.inter_op_num_threads = 1
                        _cpu_opts.execution_mode = _ort.ExecutionMode.ORT_SEQUENTIAL
                        _cpu_opts.enable_mem_pattern = True
                        _cpu_opts.enable_cpu_mem_arena = True
                        _cpu_sess = _ort.InferenceSession(
                            model_path,
                            sess_opts=_cpu_opts,
                            providers=['CPUExecutionProvider'],
                        )
                        FACE_SWAPPER.session = _cpu_sess
                        update_status(
                            f"CPU session optimised ({os.cpu_count()} threads, "
                            "mem_pattern=on, ORT_ENABLE_ALL).", NAME
                        )
                    except Exception as _cpu_e:
                        print(f"[{NAME}] CPU session tuning skipped: {_cpu_e}")
